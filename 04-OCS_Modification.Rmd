...
1. Clone or fork the case study repository from GitHub. Clone if you only want to have the repository locally. Fork if you would like to have both a local version and a remote repository on your personal GitHub account. Alternatively, use the `OCSdata` package `zip_ocs()` function to download the case study files without our git history.
